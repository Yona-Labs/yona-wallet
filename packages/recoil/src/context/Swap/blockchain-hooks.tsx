...
import { solanaClientAtom } from "../../atoms";
...
export type SwapReceipt = {
  kind: SwapReceiptKind;
  data: SwapReceiptData;
};

enum SwapReceiptKind {
  Solana = "solana",
}
type SwapReceiptData = SwapReceiptDataSolana;
export type SwapReceiptDataSolana = { signature: string };
...
export function useConfirmTransactionSwap({
  receipt,
}: {
  receipt: SwapReceipt;
}): () => Promise<void> {
  ...
  return useCallback(async () => {
    ...
    switch (swapTy) {
      case SwapType.Solana:
        await solanaClient.confirmTransaction(receipt.data.signature);
        break;
      case SwapType.Wormhole:
        // todo
        throw new Error("swap not implemented for wormhole");
