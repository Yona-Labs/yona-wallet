...
import { useQuoteContext, useSwapContext } from ".";
import type { SwapQuoteResponse, SwapReceipt } from "./blockchain-hooks";
import {
  useFetchQuote,
  useFetchTransaction,
  useSendTransaction,
...
export function useExecuteSwap({
  ...
  executeSwap: (tx?: string) => Promise<SwapReceipt>;
} {
...
// Returns true if we should disable the swap button on the swap screen
// (i.e. the button that changes the "in" token to the "out" token and
// the "out" token into the "in" token).
//
// Note that we only allow hitting the swap button if can actually perform a
// swap in that new direction.
//
// This is probably suboptimal UX. Instead, one should be able to spam that
// swap button in any case. And if the swap can't occur in that direction
// we should just tell the user.
//
export function useDisableSwapButton() {
  const { from, to } = useSwapContext();
  const { isLoading: isLoadingFromToken, fromToken } = useFromToken({
    from: to,
    to: from,
  });
  const { isLoading: isLoadingToToken, toToken } = useToToken({
    from: to,
    to: from,
  });
  const { isLoading } = useQuoteContext();
