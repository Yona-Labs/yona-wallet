        ...
        >
          ...
          <YStack space="$2">
            {changes
              .filter(
                (change) =>
                  !change.humanReadableDiff.includes("Transfer control")
              )
              .map((change) => {
                ...
                return (
                  ...
                  >
                    ...
                    {change.asset ? (
                      <XStack flexShrink={0} key={change.humanReadableDiff}>
                        <Image
                          borderRadius={
                            change.asset.isNonFungible ? "$1" : "$6"
                          }
                          source={{
                            width: 22,
                            height: 22,
    ...
    >
      <YStack space="$4">
        ...
        (evaluation?.errors.length ?? 0) > 0 ? (
          <YStack space="$3">
            {[...(evaluation?.warnings ?? []), ...customWarnings].map(
              (warning) => (
                <Warning key={warning.message.toString()} warning={warning} />
              )
            )}
...
export function BlockingWarning({
  ...
  warning,
  cancelButton = "Close",
  ignoreButton = "Ignore and proceed anyway.",
}: {
  onDeny?: () => void;
  onIgnore?: () => void;
  ignoreButton?: ReactNode;
  ...
  return (
    ...
    >
      <RequestConfirmation
        ...
        onDeny={onIgnore}
        leftButton={
          onIgnore && typeof ignoreButton === "string" ? (
            <LinkButton onPress={onIgnore} label={ignoreButton} />
          ) : typeof ignoreButton !== "string" ? (
            ignoreButton
          ) : null
        }
