...
function _CollectibleDetails({
  ...
  const size = Platform.select({ native: 340, web: 340 }) as number;
...
export type CollectibleInscriptionTableProps = {
  info?: ResponseCollectible["solana"];
  onAddressClick: (url: string) => void;
  style?: TableCoreProps["style"];
};

export function CollectibleInscriptionTable({
  info,
  onAddressClick,
  style,
}: CollectibleInscriptionTableProps) {
  const { t } = useTranslation();
  const explorer = useBlockchainExplorer(Blockchain.SOLANA);

  if (!info || (!info?.inscription && !info?.spl20)) {
    return null;
  }

  // Row components are wrapped individually to persistent the table separators
  return (
    <TableCore style={style}>
      {info.inscription ? (
        <TableRowCore
          label={t("inscription")}
          value={`#${info.inscription.order}`}
        />
      ) : null}
      {info.inscription ? (
        <TableRowCore
          label={t("inscription_data")}
          onPress={() =>
            onAddressClick(
              explorerAddressUrl(
                explorer,
                info.inscription!.dataAccount,
                DEFAULT_SOLANA_CLUSTER
              )
            )
          }
          value={
            <TableRowCoreLinkValue
              content={formatWalletAddress(info.inscription.dataAccount)}
            />
          }
        />
      ) : null}
      {info.spl20 ? (
        <TableRowCore label={t("spl20_ticker")} value={info.spl20.tick} />
      ) : null}
      {info.spl20 ? (
        <TableRowCore
          label={t("spl20_amount")}
          value={Number(info.spl20.amount).toLocaleString()}
        />
      ) : null}
    </TableCore>
  );
