...
export function CollectibleOptionsButton({
  ...
  const urls = useConnectionUrls();
  const connectionUrl = urls[activeWallet.blockchain] as string;
    ...
    <>
      ...
      >
        <PopoverMenu.Group>
          ...
          <PopoverMenu.Item
            onClick={() => {
              const url = "https://www.tensor.trade/item/" + nft.address;
              window.open(url, "_blank");
            }}
          >
            {t("view_on_tensor")}
